...
function sat_updatecitylist(maxcitiesobj, sockscityobj, citieslistobj, countrycode) {    
    ...
    for (var i in allc) {
        if (parseInt(i) < parseInt(totalc)) {
            stringout += "<li class=\"ui-state-default\"><div class='citylistitem-textwrapper' title='" +  allc[i].replace(/'/g, "\\'") + "'><div class=\"sat_flagcountry\" style=\"background-image: url('" + sat_flagfolder + countrycode + ".png')\"></div>" + allc[i] + "</div><div class=\"ui-icon ui-icon-close remove-city-bt\" onclick=\"sat_removecityfromlist('" + allc[i] + "', jQuery('.sat_maxcities'), jQuery('.sat_customcitieslist'), jQuery('.sat_citieslist'),jQuery('.sat_countriesselectbox').children('option:selected').val())\" ></div></li> ";
        }
    }
    stringout += "<br/><br/>";
...
function sat_addcustomcity(citynotfoundmsg, actualcity, sockscity, countryselect, maxcitiesobj, citieslistobj) {
    ...
    if (!(typeof window.google === 'object' && window.google.maps)) {
        ...
        // Solve without googlemaps key
        if (sockscity.val() != '') {
            sockscity.val(";" + sockscity.val());
        }
        sockscity.val(actualcity.val() + sockscity.val());
        sat_updatecitylist(maxcitiesobj, sockscity, citieslistobj, data.code);
        actualcity.val("");

    } else {

        // Solve with google maps key
        var geocoder2;
