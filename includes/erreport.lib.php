...
function render2html( $name, $type, $model, $data, $status, $free, $pro, $logged ) {
    if( $logged || ( !$logged && $free ) ):
        ...
        switch( $factor_type ): 
            ...
            case 'HTML_VALIDITY': {
                $html = render_htmlvalidity( $model, $data );
                break;
            }
            ...
            case 'HEADINGS': {
                $html = render_headings( $model, $data );
                break;
            }
            case 'SPEED_ANALYSIS': {
            ...
            case 'GOOGLEPREVIEW': {
                // TODO - TEST and FIX
                $html = render_google_preview( $data );
                break;
            }
            case 'LOCATION': {
                $html = render_location( $data );
                break;
            }
            case 'IMAGE': {
                $html = '';
...
function render_backlinks( $value ) {
    ...
    return 'Total: <strong>' . $value->total . '</strong></div><div class="clearfix col factor-special">' // trick div
...
function render_google_preview( $value ) {
    $out = '';
    if( is_object( $value ) ):
        $out .= '<h3><a href="' . $value->url_href . '" title="' . $value->title . '">' . $value->title . '</a></h3>';
        $out .= '<cite>' . $value->url . '</cite>';
        $out .= '<span>' . $value->meta_description . '</span>';
    endif;
    return empty( $out ) ? false : '<div id="google-preview-style">' . $out . '</div>';
}
 
function render_headings( $model, $value ) {
    $out = '';
    if( is_object( $value ) ):
        $out .= '<p>' . $model . ' (' . $value->total . ' items)</p>';
        $out .= '<table>'; 
        for( $i = 1; $i <= 6; $i++ ):
            $out .= $i == 1 ? '<tr>' : '';
            $out .= '<td>H' . $i . '</td>';
        ...
        for( $i = 1; $i <= 6; $i++ ):
            $out .= $i == 1 ? '<tr>' : '';
            $hd_i = (string) 'h' . $i;
            $out .= '<td>' . $value->$hd_i . '</td>';
            $out .= $i == 6 ? '</tr>' : '';
        endfor;
        $out .= '</table>';
    endif;
    return empty( $out ) ? false : '<div id="headings-style">' . $out . '</div>';
}

function render_htmlvalidity( $model, $value ) {
    $out = '';
    if( is_object( $value ) ):
        $out .= sprintf( $model, $value->errors, $value->warnings );
    endif;
    return empty( $out ) ? $value : $out;
}

function render_location( $value ) {
    $out = '';
    if( is_object( $value ) ):
        $flag = '<img alt="' . $value->country_name . '" src="' . WP_HOME . '/content/themes/eranker/img/flags/flags-iso/shiny/24/' . $value->country_code . '.png" width="24" />';
        $out .= $value->city . ', ' . $value->country_name . ' (' . $value->country_code . ') ' . $flag . '<br>' 
             . $value->state . ', ' . $value->zip;
    endif;
    return empty( $out ) ? false : '<div id="location-style">' . trim( $out, " ," ) . '</div>';
}
